...
create_table = 'create table links (link_id integer, user_id integer, time integer, votes integer, title text, url text)'
insert_table = 'insert into links values (?, ?, ?, ?, ?, ?)'
select_table = ['select votes from links where link_id = 2',
				'select link_id from links where user_id = 62443 and votes > 1000',
				'select link_id from links where user_id = 62443 order by time asc']
